...
<div class="profile-card-style-5 fl-node-<?php echo esc_attr($id); ?>" style="background-image:url(<?php if ($settings->profile_bg_image == '') { echo esc_url(CEBB_URL . 'assets/images/default.png'); } else { echo esc_url($settings->profile_bg_image_src); } ?>);">
    <div class="ct-team-member-layout ">
        <div class="ct-team-member-inner">
            <div class="ct-team-image">
                ...
                <?php } else { ?>
                    <img src="<?php echo esc_url($settings->profile_image_src); ?>" width="255" height="320" class="img img-responsive">
                <?php } ?> 
                <!-- social icon -->
                ...
                <?php if ($settings->profile_icon_show_hide == 'profile_icon_yes') { ?>
                    ...
                    foreach ( $settings->profile_icons as $icon ) {
                        ...
                        if ( ! empty( $icon->connections->link ) && empty( $icon->link ) ) {
                            ...
                            echo '<a class="beaver-icon beaver-social-icon ' . esc_attr($classes[0]). ' beaver-social-icon-' . esc_attr( $icon_count ) . '" href="' . esc_url($icon->link) . '" target="' . esc_attr( $icon->link_target ) . '">'; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
...
<style>
.profile-card-style-5.fl-node-<?php echo esc_attr($id); ?> .team-member__socialmedia .beaver-icon i,.profile-card-style-2.fl-node-<?php echo esc_attr($id); ?> .team-member__socialmedia .beaver-icon i:before{ 
    /* Icon Color */
...
.profile-card-style-5.fl-node-<?php echo esc_attr($id); ?> .team-member__socialmedia .beaver-icon i:hover,
.profile-card-style-5.fl-node-<?php echo esc_attr($id); ?> .team-member__socialmedia .beaver-icon i:hover:before {
...
.profile-card-style-5.fl-node-<?php echo esc_attr($id); ?> .beaver-icon:not(:last-child){
    margin-right:<?php echo esc_attr($settings->profile_icon_spacing); ?>px; 
}
<?php  if ($settings->profile_icon_show_hide == 'profile_icon_yes') { ?>
.profile-card-style-5 .ct-team-member-layout:hover .team-member__socialmedia {
    bottom: 30px;
    opacity: 1;
}
<?php } else{?>
